...
init :: { max :: Int, coolOffPeriod :: Int, speed :: Number, rate :: Int, power :: Number } -> Exists (EntityBehaviour EntityCommand GameEvent)
init { max, speed, coolOffPeriod, rate, power } = mkExists $ EntityBehaviour { state: { firingTimer: 0
                                                                                      , coolOffTimer: 0
                                                                                      , bulletsFired: 0
                                                                                      , power
                                                                                      , firing: false
             ...
             where handleCommand command state@{ firingTimer } = do
                                             entity <- B.entity 
                                             case command of
                                               StartFireBullet -> 
